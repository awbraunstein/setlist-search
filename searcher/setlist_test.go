...
func TestParseSetlist(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid setlist",
			setlist: "ID{1}URL{http://phish.net.com/setlists/blah}DATE{2000-07-20}SET1{a,b,c,d}SET2{x,y,z}ENCORE{aa,bb}",
			expected: &Setlist{
				ShowId: 1,
				Date:   "2000-07-20",
				Url:    "http://phish.net.com/setlists/blah",
				Sets: []*Set{
...
func TestParseSetlistFromPhishNet(t *testing.T) {
	...
	got, gotss, err := ParseSetlistFromPhishNet(&gophish.Setlist{
		ShowId:      1,
		ShowDate:    "2000-04-20",
		Url:         "http://phish.net/setlists/phish-december-29-1997-madison-square-garden-new-york-ny-usa.html",
